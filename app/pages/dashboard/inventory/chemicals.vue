  ...
  <div class="dash-page">
    <div class="dash-head">
      ...
      <div class="flex-1 min-w-0">
        <h1 class="dash-title bitter">Препарати і добрива</h1>
        <p class="dash-subtitle">Залишки та облік хімікатів</p>
      </div>
      ...
      <div v-for="item in items" :key="item.id" class="card flex flex-col gap-3">
        <div class="flex items-start justify-between">
          <div class="flex-1 min-w-0">
            ...
            <p v-if="item.farm" class="text-xs text-agro-light mt-0.5 flex items-center gap-1">
        ...
        <p v-if="item.last_log" class="text-xs text-agro-light">
          Останнє: {{ item.last_log.quantity }} {{ item.unit }} — {{ formatDate(item.last_log.created_at) }}
        </p>
      </div>
    ...
    <div v-if="showAdd" class="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-4" @click.self="showAdd = false; resetForm()">
      <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
        ...
        <div class="space-y-4">
          <div class="relative">
            ...
            <div v-if="productSuggestions.length" class="absolute z-50 left-0 right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-agro-border max-h-48 overflow-y-auto">
              <button v-for="p in productSuggestions" :key="p.id" @click="selectProduct(p)" class="w-full text-left px-4 py-2.5 text-sm hover:bg-agro-hover transition-colors border-b border-agro-border last:border-0">
          ...
          <div>
            <label class="block text-sm font-medium text-agro-dark mb-1">Мінімальний залишок <span class="text-agro-light font-normal">(сповіщення якщо менше)</span></label>
            <div class="relative">
        ...
        <div class="flex gap-3 mt-6">
          ...
          <button @click="addItem" :disabled="saving || !form.name || !form.quantity" class="flex-1 btn-primary inline-flex items-center justify-center gap-1.5">
            {{ saving ? '...' : 'Додати' }}
          </button>
        </div>
      </div>
...
useHead({ title: 'Препарати і добрива — Склад' })
...
const resetForm = () => { form.value = { name: '', quantity: 0, unit: 'л', farm_id: '', min_quantity: null }; productSuggestions.value = [] }
...
const onProductSearch = () => {
  ...
  searchTimer = setTimeout(async () => {
    const { data } = await supabase.from('agro_products').select('id, name, formulation_name').ilike('name', `%${q}%`).limit(8)
    productSuggestions.value = data || []
  }, 250)
}
const selectProduct = (p: any) => { form.value.name = p.name; productSuggestions.value = [] }

async function load() {
  loading.value = true
  const uid = await getQueryUserId()
  if (!uid) { loading.value = false; return }
  const [{ data: inv }, { data: farmsData }] = await Promise.all([
    supabase.from('farm_inventory').select('*, farm:farms(id, name)').eq('user_id', uid).order('name'),
    supabase.from('farms').select('id, name').eq('user_id', uid).order('name'),
  ])
  const invList = inv || []
  if (invList.length) {
    const ids = invList.map(i => i.id)
    const { data: logs } = await supabase.from('farm_inventory_log').select('inventory_id, type, quantity, created_at').in('inventory_id', ids).order('created_at', { ascending: false })
    const lastLogMap: Record<string, any> = {}
    for (const log of logs || []) { if (!lastLogMap[log.inventory_id]) lastLogMap[log.inventory_id] = log }
    items.value = invList.map(i => ({ ...i, last_log: lastLogMap[i.id] || null }))
  } else { items.value = [] }
