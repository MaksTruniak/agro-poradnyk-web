    ...
    <div v-else-if="product" :class="MARKETPLACE ? 'grid xl:grid-cols-3 gap-8' : 'grid gap-8'">
      ...
      <div :class="MARKETPLACE ? 'xl:col-span-2 space-y-6' : 'space-y-6'">
        <div class="card">
          ...
          <div v-if="product.formulation_name || product.chem_class" class="mt-4 flex flex-wrap gap-3">
            <div v-if="product.formulation_name" class="text-sm">
              <span class="text-agro-light">Препаративна форма:</span>
              <span class="font-medium text-agro-dark ml-1">{{ product.formulation_name }}</span>
            </div>
            <div v-if="product.chem_class" class="text-sm">
              <span class="text-agro-light">Хімічний клас:</span>
              <span class="font-medium text-agro-dark ml-1">{{ product.chem_class }}</span>
            </div>
          </div>
...
const { data: pageData, pending } = useLazyAsyncData(`catalog-${slug}`, async () => {
  const [productData, analogsData] = await Promise.all([
    ...
    api.getAnalogs(slug).catch(() => ({ analogs: [] })),
  ])
  const product = productData?.product || null
  const analogsList = analogsData?.analogs || []
  return { product, analogs: analogsList }
})
