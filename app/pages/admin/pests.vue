  ...
  <div class="p-8">
    <div class="mb-6">
      <h1 class="text-2xl font-extrabold text-agro-dark flex items-center gap-2">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="13" rx="5" ry="6"/><path d="M12 7V3"/><path d="M7 14H4M20 14h-3"/><path d="M6 8l-2-2M18 8l2-2"/></svg>
        Шкідники
      </h1>
      ...
      <div class="divide-y divide-agro-border">
        <div v-for="item in items" :key="item.id" class="flex items-center gap-3 px-5 py-3 hover:bg-agro-bg transition-colors">
          <div class="w-10 h-10 rounded-xl bg-agro-hover flex items-center justify-center shrink-0 overflow-hidden">
            ...
            <svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgb(107,122,100)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="13" rx="5" ry="6"/><path d="M12 7V3"/><path d="M7 14H4M20 14h-3"/></svg>
          </div>
          <div class="flex-1 min-w-0">
            <p class="font-semibold text-agro-dark truncate text-sm">{{ item.name }}</p>
            <p class="text-xs text-agro-light truncate">{{ pestCategories.find(c => c.slug === item.category_slug)?.name || item.category_slug || item.slug }}</p>
          </div>
      ...
      <div v-if="drawer.show" class="fixed inset-0 z-50 flex justify-end bg-black/20" @click.self="drawer.show = false">
        <div class="bg-white w-full max-w-md h-full flex flex-col shadow-2xl">
          <div class="px-6 pt-6 pb-4 border-b border-agro-border flex items-start justify-between gap-3 shrink-0">
            <div>
              <p class="text-xs text-agro-light mb-0.5">Шкідник</p>
          ...
          <div class="px-6 py-4 border-t border-agro-border shrink-0">
            <p class="text-xs text-agro-light">{{ drawer.products.length }} препаратів проти цього шкідника</p>
          </div>
        </div>
      </div>
    </Transition>
      ...
      <div v-if="modal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20" @click.self="modal.show = false">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-white rounded-t-3xl px-6 pt-6 pb-4 border-b border-agro-border flex items-center justify-between">
            <h3 class="font-bold text-agro-dark text-lg">{{ modal.isNew ? 'Новий шкідник' : 'Редагувати' }}</h3>
          ...
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model="modal.form.name" @input="autoSlug" class="input text-sm" placeholder="Попелиця злакова" />
              </div>
              <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model="modal.form.latin_name" class="input text-sm italic" placeholder="Schizaphis graminum" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-agro-dark mb-1">Ряд / клас</label>
                <input v-model="modal.form.order_name" class="input text-sm" placeholder="Hemiptera" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-semibold text-agro-dark mb-1">Культура</label>
                <input v-model="modal.form.culture" class="input text-sm" placeholder="Зернові, кукурудза" />
              </div>
              <div>
                ...
                <select v-model="modal.form.category_slug" class="input text-sm">
                  ...
                  <option v-for="cat in pestCategories" :key="cat.slug" :value="cat.slug">{{ cat.name }}</option>
                </select>
              </div>
            </div>
            <div>
    ...
    <Transition name="fade">
      <div v-if="deleteModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20" @click.self="deleteModal.show = false">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6">
          ...
          <p class="font-bold text-agro-dark text-lg mb-1">Видалити шкідника?</p>
...
useHead({ title: 'Шкідники — Адмін' })
definePageMeta({ layout: 'admin', middleware: 'admin' })

const supabase = useSupabaseClient()
const api = useAgroApi()
const pestCategories = ref<{ slug: string; name: string }[]>([])
const LIMIT = 40
const search = ref('')
const page = ref(1)
const total = ref(0)
...
const load = async () => {
  loading.value = true
  const { items: data, total: t } = await api.getPests({ q: search.value || undefined, limit: LIMIT, offset: (page.value - 1) * LIMIT })
  items.value = data
  total.value = t
  loading.value = false
}

const emptyForm = () => ({ name: '', slug: '', latin_name: '', order_name: '', culture: '', category_slug: '', source_image_url: '', description: '' })
...
const openModal = (item: any) => {
  ...
  modal.form = item ? { name: item.name ?? '', slug: item.slug ?? '', latin_name: item.latin_name ?? '', order_name: item.order_name ?? '', culture: item.culture ?? '', category_slug: item.category_slug ?? '', source_image_url: item.source_image_url ?? '', description: item.description ?? '' } : emptyForm()
  modal.show = true
}
...
const save = async () => {
  ...
  if (modal.isNew) {
    const { data, error } = await supabase.from('agro_pests').insert(payload).select().single()
    if (error) { saveError.value = error.message; modal.saving = false; return }
    if (data) { items.value.unshift(data as any); total.value++ }
  } else {
    const { error } = await supabase.from('agro_pests').update(payload).eq('id', modal.id)
    if (error) { saveError.value = error.message; modal.saving = false; return }
...
const openProducts = async (item: any) => {
  ...
  drawer.show = true
  try {
    const data = await api.getPest(item.slug)
    drawer.products = data.pest?.products || []
  } catch {}
...
const doDelete = async () => {
  ...
  await supabase.from('agro_pest_products').delete().eq('pest_id', deleteModal.item.id)
  await supabase.from('agro_pests').delete().eq('id', deleteModal.item.id)
...
onMounted(async () => {
  const { data } = await supabase.from('agro_categories').select('slug, name').eq('section', 'pests').eq('is_active', true).order('sort_order')
  pestCategories.value = data || []
  load()
})
</script>
