...
async function run() {
  // Завантажуємо всі бур'яни з БД
  const { data: weeds } = await supabase.from('agro_weeds').select('id, slug')
  console.log(`📋 Бур'яни: ${weeds.length}`)

  // Завантажуємо всі продукти (пагінація бо >1000)
  const allProducts = []
  let pg = 0
  while (true) {
  ...
  for (let i = 0; i < weeds.length; i++) {
    const weed = weeds[i]
    process.stdout.write(`\r  ${i + 1}/${weeds.length} (${linked} зв'язків)`)

    const { weed: detail } = await apiFetch(`/v1/weeds/${weed.slug}`)
    if (!detail?.herbicides?.length) continue

    const rows = detail.herbicides
      .map(h => ({ weed_id: weed.id, product_id: productBySlug[h.slug] }))
      .filter(r => r.product_id)

    missing += detail.herbicides.length - rows.length

    if (rows.length) {
      await supabase.from('agro_weed_herbicides').upsert(rows, { onConflict: 'weed_id,product_id' })
      linked += rows.length
    }
  ...
  console.log(`\n\n✅ Готово: ${linked} зв'язків збережено, ${missing} гербіцидів не знайдено в БД`)
}
